...
**Fix commands:**
- `php artisan olm:migrate-tag {retired} {desired} [--type=] [--category=] [--apply]` — Applies one approved tag mapping (replaced `olm:fix-orphaned-tags`, which was deleted). Dry run by default; see `readme/PostTagMigrationClean.md`.
...
**Key files:**
- `app/Console/Commands/tmp/v5/PostMigAug2026/MigrateTag.php`
