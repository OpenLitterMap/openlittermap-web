...
- Email deliverability: `App\Support\EmailAddress::isSendable()` is the SINGLE source of truth for "is this address worth sending to" (filter_var + dotted-domain, no DNS) — used by `SubscribersController` AND the `olm:send-email-to-subscribed` dispatch guard. Never re-inline the regex
- `email_suppressions` is the source of truth for undeliverable addresses (one row per normalized email; **complaint outranks bounce, never downgrade** — use `EmailSuppression::suppress()`). `email_sends` is the campaign ledger: `uniq(campaign, email)` is the atomic claim lock, re-runs skip `accepted`/in-flight `queued` and re-evaluate previously-skipped rows. All `email` columns store the normalized address (`strtolower(trim())`)
- `DispatchEmail(?int $ledgerRowId, EmailRecipient $recipient)` writes ledger `accepted`/`failed` only in its terminal hooks (`handle()`/`failed()`), never on first throw. `EmailRecipient` (not a `User`) carries the right `sub_token`. Attempts are time-based (`retryUntil` +6h) + `maxExceptions`, NOT a fixed `$tries` — `RateLimited` releases increment attempts, so a fixed `$tries` would strand throttled recipients as `failed`
- SNS webhook `POST /webhooks/aws/ses/sns` (CSRF-exempt via `webhooks/*`): verify SNS signature + `TopicArn` (`config('services.ses.topic_arn')`) BEFORE anything, read the raw `text/plain` body, idempotent on `(sns_id, email)`. The `MessageValidator` is container-injected so tests can swap it
- `users.email_verified_at` is the new (forward-compat) verification timestamp — `User::confirmEmail()` dual-writes it alongside the deprecated `users.verified` boolean. Bulk-send eligibility is still `emailsub=1` only (includes `verified=0`)
