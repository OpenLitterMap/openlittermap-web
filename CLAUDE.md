...
- `DispatchEmail(?int $ledgerRowId, EmailRecipient $recipient)` writes ledger `accepted`/`failed` only in its terminal hooks (`handle()`/`failed()`), never on first throw. `EmailRecipient` (not a `User`) carries the right `sub_token`
