...
class TotalDataController extends Controller
{
    ...
    {
        if ($user = Auth::user()) {
            if ($user->email == 'seanlynch@umail.ucc.ie') {
                Excel::create('cigarette_butts', function ($excel): void {
                    $excel->sheet('OLM', function ($sheet): void {
                        $index = 0;
                        $export = [];
