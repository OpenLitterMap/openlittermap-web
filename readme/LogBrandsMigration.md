...
The v5 migration extracts and validates **2,452 unique brands** from two sources into a unified system with AI-validated brand-object relationships.

## Current Files

### 1. LogBrandRelationships.php
Extracts brand-object relationships from photos. Key points:
- **IMPORTANT**: Does NOT normalize brand keys to lowercase (preserves "McDonald's", "7-Eleven", etc.)
- Extracts from both official brands table and custom tags
- Outputs CSV with brand-object pairs and their frequencies

### 2. ValidateBrands.php
Command to validate brand-object relationships using OpenAI API:
- Can process by brand, letter, or minimum photo count
- `--min-photos=500` works as standalone filter
- Costs ~$0.005 per brand
- Outputs validation results to JSON files

### 3. BrandValidator.php
Service class that handles the actual AI validation:
- Sends brand-object pairs to OpenAI API
- Uses percentage thresholds (80%+ is strong signal for valid relationship)
- Determines if brand manufactures/sells the object
- Saves results to `storage/app/brand-validations/`

### 4. GenerateBrandsConfig.php
Creates the final configuration file from validation results.

## The Problem We're Solving

We have brands stored in two places:
1. **Official brands table**: 100 brands linked via `photos.brands_id`
2. **Custom tags**: 2,383 brands stored as `brand:xxx` or `brand=xxx` tags

These need to be:
- Extracted with their exact keys (case-sensitive)
- Validated to confirm brand actually makes/sells the object
- Consolidated into a single configuration

## Key Issue Fixed: Brand Key Preservation

The original code normalized all brand keys to lowercase, losing ~52 brands:
```php
// BAD - loses brands
$normalized = strtolower(trim($brandKey));

// GOOD - preserves exact keys
return trim($brandKey);
```

## Validation Logic

The AI validates based on:
- **Manufacture/Sell relationship**: Brand must make or sell the item
- **Percentage thresholds**:
    - 80-100% = Very likely valid
    - 60-79% = Probably valid
    - <20% = Likely coincidental

Example validations:
- ✅ `adidas + other.clothing` (they make clothing)
- ❌ `adidas + softdrinks.energy_can` (they don't make drinks)
- ✅ `unknown_brand + waterbottle (100%)` (likely a water brand)

## What's Next

### Step 1: Extract Brand Relationships
```bash
php artisan olm:extract-brands
```
Should show ~2,452 unique brands (not 2,400)

### Step 2: Validate Brands
Start with high-volume brands to test:
```bash
# Test with one brand
php artisan olm:validate-brands --brand=redbull

# Process high-volume brands (>500 photos)
php artisan olm:validate-brands --min-photos=500
...
```bash
php artisan olm:generate-brands-config
```
Creates `BrandsConfig_generated.php` with validated relationships

### Step 4: Review and Deploy
1. Review generated config for obvious errors
2. Copy to `app/Tags/BrandsConfig.php`
3. Run the actual v5 migration

## Cost Estimates

| Brands | Estimated Cost | Time |
|--------|---------------|------|
| 12 brands (1000+ photos) | $0.06 | ~12 seconds |
| 30 brands (500+ photos) | $0.15 | ~30 seconds |
| 100 brands (100+ photos) | $0.50 | ~2 minutes |
| All 2,452 brands | $12.50 | ~40 minutes |

## Files Generated

- `brand-relationships-*.csv` - All brand-object pairs
- `brand-validations/*.json` - Individual validation results
- `BrandsConfig_generated.php` - Final configuration

## Verification Query

Check extraction worked correctly:
```sql
SELECT COUNT(DISTINCT key) as unique_brands
FROM brandslist;
-- Should return 2,452
```
