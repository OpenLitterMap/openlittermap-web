...
**Response 422:** Validation error. Rejects the entire payload if any `clo_id` is stale (`One or more of these tags is no longer available — refresh your tag list.`) or resolves to a retired litter object. The retired check runs inside the transaction, ahead of the delete, so a refusal leaves the user's existing presets untouched.
