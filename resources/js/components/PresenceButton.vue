<template name="presence-button">
    <div class="expand-mobile">
        <strong :style="{ color: remaining ? 'green' : 'red' }"><slot>{{ remainingText }}</slot></strong>
        <br>
        <button class="button" :class="{ 'is-danger': remaining, 'is-success': ! remaining }" @click="togglePresence"><slot v-if="remaining">It's still there.</slot>
        <slot v-else>I picked it up!</slot></button>
    </div>
</template>
