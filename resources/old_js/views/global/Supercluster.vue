...
function createArtIcon(feature, latlng) {
    ...
    return feature.properties.verified === 2 ? L.marker(x, { icon: green_dot }) : L.marker(x, { icon: grey_dot });
}
...
function createClusterIcon(feature, latlng) {
    ...
    const icon = L.divIcon({
        html: '<div class="mi"><span class="ma">' + feature.properties.count + '</span></div>',
...
function onEachFeature(feature, layer) {
    if (feature.properties.cluster) {
        layer.on('click', function (e) {
            const zoomTo = map.getZoom() + ZOOM_STEP > MAX_ZOOM ? MAX_ZOOM : map.getZoom() + ZOOM_STEP;
...
function onEachCleanup(feature, layer) {
    layer.on('click', function (e) {
        ...
        L.popup(mapHelper.popupOptions).setLatLng(latLng).setContent(content).openOn(map);
    });
}
...
function getActiveLayers() {
    ...
    pointsLayerController._layerControlInputs.forEach((lyr, index) => {
        if (lyr.checked) {
            ...
            const name =
                pointsLayerController._layers[index].name.toLowerCase() === 'petsurprise'
                    ? 'dogshit'
...
export default {
    ...
    mounted() {
        ...
        map.attributionControl.addAttribution(
            'Litter data &copy OpenLitterMap & Contributors ' + year + ' Clustering @ MapBox'
        );
        ...
        if (this.activeLayer === 'cleanups') {
            ...
            if (this.$route.params.hasOwnProperty('invite_link')) {
                ...
                const userId = this.$store.state.user.auth ? this.$store.state.user.user.id : null;
    ...
    methods: {
                ...
                await axios
                    ...
                    .then((response) => {
                        ...
                        points = glify.points({
                            ...
                            click: (e, point, xy) => {
                                const feature = response.data.features.find((f) => {
                                    return (
                                        f.geometry.coordinates[0] === point[0] && f.geometry.coordinates[1] === point[1]
                                    );
                                });
                        ...
                        if (photoId) {
                            ...
                            if (feature) {
                                this.renderLeafletPopup(feature, [
        ...
        updateUrlPhotoIdAndFlyToLocation(location) {
            ...
            const flyDistanceInMeters = map.distance(map.getCenter(), [location.latitude, location.longitude]);
        ...
        setupYearDropdown() {
            ...
            let years = [{ label: 'All Time', value: '*' }];
