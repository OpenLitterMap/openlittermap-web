...
class CreateCSVExportTest extends TestCase
{
    ...
    /**
     * Regression: the v5 migration wrote tags onto litter_objects that have no
     * category_litter_object pivot row (e.g. `plasticBags`). Columns used to be derived from
     * the pivot, so those tags got no column at all and their quantities vanished from every
     * export — 277k items on production.
     * A tagged object must always get a column, pivot row or not.
     */
    public function test_tagged_object_without_a_category_pivot_still_gets_a_column()
    {
        ...
        $this->assertSame(
            ...
            'fixture invalid: object must have no pivot row'
        );
        ...
        $photo = Photo::factory()->create([
            ...
            'summary' => [
                'tags' => [
                    ['clo_id' => null, 'category_id' => $category->id, 'object_id' => $shadow->id, 'type_id' => null, 'quantity' => 603, 'materials' => [], 'brands' => (object) [], 'custom_tags' => []],
                ],
                'totals' => ['litter' => 603, 'materials' => 0, 'brands' => 0, 'custom_tags' => 0],
                'keys' => ['brands' => []],
            ],
        ]);
        ...
        $this->assertContains('plasticBags', $headings, 'tagged object with no pivot row was dropped from the CSV');

        $mapped = $export->map($photo->fresh());
        $index = array_search('plasticBags', $headings);

        $this->assertEquals(603, $mapped[$index], 'quantity was silently lost from the export');
    }

    /**
     * Joined-format headings are synthesized as "{type}_{object}", so a pivotless migrated
     * object whose key is literally `beer_can` collides with canonical object `can` + type
     * `beer`. Two physically distinct columns end up sharing one header, and associative CSV
     * readers silently drop one of them — recreating the undercount the pivot fix removed.
     *
     * Only the legacy side may be renamed: `beer_can` resolves today and consumers read it.
     */
    public function test_joined_headings_disambiguate_legacy_and_canonical_collisions()
    {
        $category = Category::where('key', 'alcohol')->firstOrFail();
        $can = LitterObject::where('key', 'can')->firstOrFail();
        $beerType = LitterObjectType::where('key', 'beer')->firstOrFail();

        $canCloId = CategoryObject::where('category_id', $category->id)
            ->where('litter_object_id', $can->id)
            ->value('id');

        // Shadow object whose raw key collides with the synthesized "{beer}_{can}" header.
        $shadow = LitterObject::firstOrCreate(['key' => 'beer_can'], ['crowdsourced' => true]);
        ...
        $photo = Photo::factory()->create([
            ...
            'summary' => [
                'tags' => [
                    ['clo_id' => $canCloId, 'category_id' => $category->id, 'object_id' => $can->id, 'type_id' => $beerType->id, 'quantity' => 10, 'materials' => [], 'brands' => (object) [], 'custom_tags' => []],
                    ['clo_id' => null, 'category_id' => $category->id, 'object_id' => $shadow->id, 'type_id' => null, 'quantity' => 20, 'materials' => [], 'brands' => (object) [], 'custom_tags' => []],
                ],
                'totals' => ['litter' => 30, 'materials' => 0, 'brands' => 0, 'custom_tags' => 0],
                'keys' => ['brands' => []],
            ],
        ]);
        ...
        PhotoTag::create([
            ...
            'litter_object_type_id' => $beerType->id,
            'quantity' => 10,
        ]);
        PhotoTag::create([
            'photo_id' => $photo->id,
        ...
        $export = new CreateCSVExport(null, null, null, $user->id, [], [], ['joined']);
        $headings = $export->headings();

        $duplicates = array_keys(array_filter(array_count_values($headings), fn ($n) => $n > 1));
        $this->assertSame([], $duplicates, 'joined export emitted duplicate column headers: ' . implode(', ', $duplicates));

        $mapped = $export->map($photo->fresh());
        $values = [];
        foreach ($headings as $i => $h) {
            if (str_contains($h, 'beer_can')) {
                $values[$h] = $mapped[$i];
            }
        }

        // The canonical typed column KEEPS its established header. Renaming both sides would
        // break every existing consumer reading `beer_can`, which works today.
        $this->assertArrayHasKey('beer_can', $values, 'canonical can+beer column lost its header');
        $this->assertEquals(10, $values['beer_can'], 'canonical can+beer quantity missing');

        // Only the legacy bare-object column is renamed, and stays addressable.
        $legacy = array_diff_key($values, ['beer_can' => null]);
        $this->assertCount(1, $legacy, 'expected exactly one renamed legacy column');
        $this->assertSame("legacy__beer_can__{$shadow->id}", array_key_first($legacy));
        $this->assertEquals(20, reset($legacy), 'legacy beer_can quantity missing');
    }
